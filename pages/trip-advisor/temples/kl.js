...
function TempleInfo({ name }) {
    ...
    // console.log(templeData,'kk');
    // console.log(td[name][0].templeInfo,'000000000');
    
    return (
        <div className="space-y-4">
            <p className="text-lg text-gray-800 leading-relaxed">
                {name && td[name][0].templeInfo}
            </p>
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-pink-50 rounded shadow capitalize">
                    <h4 className="font-semibold text-pink-600">Highlight</h4>
                    <p> {name && td[name][0].highlight}</p>
                </div>
                <div className="p-4 bg-pink-50 rounded shadow">
                    <h4 className="font-semibold text-pink-600">District</h4>
                    <p> {name && td[name][0].district}</p>
                </div>
            </div>
        </div>
    );
...
function SevaDarshanam({ name }) {
    const sevaList = name && td[name][0].sevas
    ...
    const handleSubmit = (index) => {
        ...
        const templename = name && td[name][0].name;
...
function PlacesToVisit({ name  }) {
    const places = td['kanaka-durga-temple'][0].placesToVisit
    // console.log(data, 'places to visit data');
...
export default function TempleDetailPage() {
    ...
    // console.log(router.query.templename, 'temple name');
        ...
        <>
            ...
            <div className="flex min-h-screen bg-gray-100">
                ...
                {/* <aside className="w-64 bg-white shadow p-4 space-y-2">
                    <h2 className="text-xl font-bold text-[#D5451B] mb-4">Temple Details</h2>
                    {tabs.map((tab) => (
                        <button
                            key={tab.key}
                ...
                <main className="flex lg:px-20 p-6">
                    <div className="flex flex-col lg:flex-row gap-6 items-start">
                        <div className="w-full lg:w-1/3">
                            <Image
                                src={router.query.templename && td[router.query.templename][0].image}
                                alt={templeData.name}
                                width={400}
                                height={400}
                                className="rounded"
                            ...
                            <div className="mt-4 flex flex-col gap-y-1">
                                <h1 className="text-2xl font-bold text-gray-800 capitalize">{router.query.templename && td[router.query.templename][0].name}</h1>
                                <p className="text-sm text-gray-600 italic">{router.query.templename && td[router.query.templename][0].district}</p>
                                <Link href={`tel:${router.query.templename && td[router.query.templename][0].mobileno}`} className="text-sm text-gray-800 ">{router.query.templename && td[router.query.templename][0].mobileno}</Link>
                            </div>
                        ...
                        <div className="w-full lg:w-2/3 bg-white rounded-lg shadow p-6">
                            ...
                            {activeTab === 'info' && <TempleInfo name = {router.query.templename} />}
                            {activeTab === 'seva' && <SevaDarshanam name = {router.query.templename} />}
                            {activeTab === 'facilities' && <Facilities name = {router.query.templename}  />}
                            {activeTab === 'places' && <PlacesToVisit name = {router.query.templename}/>}
                            {activeTab === 'itinerary' && <ItineraryDetails name = {router.query.templename}  />}
                        </div>
                    </div>
                </main>
            </div>
