...
export default function FilterBar() {
  ...
  const [sortOrder, setSortOrder] = useState('Sort: Low to High');

  const bhkOptions = ['All BHK', '1 BHK', '2 BHK', '3 BHK', '4 BHK', '5 BHK', '6 BHK', '7 BHK'];
  const priceOptions = ['All Prices', '6K - 8K', '8K - 10K', '10K - 12K', '12K - 14K', '14K - 18K'];
  const sortOptions = ['Sort: Low to High', 'Sort: High to Low'];

  const clearFilters = () => {
    setSearchTerm('');
    setSelectedBHK('All BHK');
    setPriceRange('All Prices');
    setSortOrder('Sort: Low to High');
  };

  const CustomDropdown = ({ selected, setSelected, options }) => {
    ...
    return (
      <div className="relative w-36">
        <button
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-md text-sm bg-white text-left"
        >
          {selected}
        </button>
        {show && (
  ...
  return (
    <div className="w-full lg:pl-16 py-4 px-4 flex flex-wrap gap-4 justify-start text-black">
      <input
        type="text"
      ...
      <button
        ...
        className="px-4 py-2 border border-gray-300 hover:bg-red-300 rounded-md text-sm flex gap-x-1 justify-center items-center h-10"
      >
